                    ...
                    <div class="svg-container">
                        <div class id="edge">
                            <svg preserveAspectRatio="xMidYMid meet" viewBox="0 0 800 650" class="svg-content-responsive">
                                <g :transform="style">
                                    <g>
                                        <path class="link" v-for="(b, i) in getLinks" :key="i" :d="getLines(b)" />
                                    </g>
                                    <g>
                                        <text class="node" v-for="d in getNodes" :key="d.id" :dy="d.dy" :text-anchor="d.textAnchor" :transform="d.transform" :style="{ fontSize: fontSize + 'px' }">{{ d.text }}</text>
                                    </g>
                                </g>
                            </svg>
                        </div>
                    ...
                    <v-expansion-panel>
                        ...
                        <v-expansion-panel-content>
                            <div class="d-flex">
                                ...
                                <v-slider v-model="updateDiameter" class="mt-2" max="900" min="600" :size="diameter" thumb-label></v-slider>
                            </div>
                            <div class="d-flex mt-n4">
                                <v-subheader class="pl-0" style="width:7em;">Tension:</v-subheader>
                                <v-slider v-model="getTension" max="10" min="1" :size="diameter" thumb-label class="mt-2"></v-slider>
                            </div>
                            <div class="d-flex mt-n4">
                                <v-subheader class="pl-0" style="width:7em;">Text Offset:</v-subheader>
                                <v-slider v-model="textOffset" max="15" min="4" :size="diameter" thumb-label class="mt-2"></v-slider>
                            </div>
                            <div class="d-flex mt-n4">
                                <v-subheader class="pl-0" style="width:7em;">Text Size:</v-subheader>
                                <v-slider v-model="fontSize" max="12" min="8" :size="diameter" thumb-label class="mt-2"></v-slider>
                            </div>
                        </v-expansion-panel-content>
                    </v-expansion-panel>
                </v-expansion-panels>
        ...
        async fetchData() {
            ...
            data = data.map((d, i) => {
                let breaker = d.To.split(",").map(function(item) {
                    item = item.trim();
                    let searchType = records.find(element => element.email === item);
                    if (searchType) {
        ...
        drawNodes() {
            ...
            if (root != undefined) {
                ...
                return root.leaves().map((d, i) => {
                    ...
                    let transform = "rotate(" + (d.x - 90) + ")translate(" + (d.y + this.textOffset) + ",0)" + (d.x < 180 ? "" : "rotate(180)");
                    return {
                        id: i + 1,
                        dy: "0.28em",
