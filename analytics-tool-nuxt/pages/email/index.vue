        ...
        <v-expansion-panels>
          <v-expansion-panel>
            ...
            <v-expansion-panel-content>
              <div class="d-flex">
                <v-subheader class="pl-0" style="width:7em;"
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      this.datePicker.limitMax = moment.max(stringOFDates).format("YYYY-MM-DD");
    ...
    draw(data) {
      if (data) {
        ...
        this.formatedData = this.formatData(this.original, this.clusterData);
      }
    ...
    formatData(data, records) {
      ...
      let fromData = data.map(d => {
        if (!mySet.has(d.From)) {
          mySet.add(d.From);
          let searchType = records.find(element => element.email === d.From);
          results.push({
            id: d.From,
        ...
        let breaker = d.To.split(",").map(function(word) {
          ...
          if (!mySet.has(word)) {
            mySet.add(word);
            let searchType = records.find(element => element.email === word);
            results.push({
              id: word,
              group: searchType["type"]
            });
